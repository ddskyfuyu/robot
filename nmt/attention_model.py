...
class AttentionModel(model.Model):
    ...
    def __init__(self,
        ...
        super(AttentionModel, self).__init__(hparams=hparams,
                                             mode=mode,
                                             iterator=iterator,
    ...
    def _prepare_beam_search_decoder_inputs(self, beam_width, memory, source_sequence_length, encoder_state):
        ...
        fbatch_size = self.batch_size * beam_width
    ...
    def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state, source_sequence_length):
        ...
        if not self.has_attention:
            return super(AttentionModel, self)._build_decoder_cell(hparams, encoder_outputs, 
        ...
        if (self.mode == tf.contrib.learn.ModeKeys.INFER and infer_mode == "beam_search"):
            memory, source_sequence_length, encoder_state, batch_size = (
                                   self._prepare_beam_search_decoder_inputs(hparams.beam_width, 
                                                                            memory, 
        ...
        # Attention
        attention_mechanism = self.attention_mechanism_fn(hparams.attention, num_units, 
                                                      memory, source_sequence_length, self.mode)
        ...
        alignment_history = (self.mode == tf.contrib.learn.ModeKeys.INFER and infer_mode != "beam_search")
...
def create_attention_mechanism(attention_option, num_units, memory, source_sequence_length, mode):
    ...
    if attention_option == "luong":
        ...
        attention_mechanism = tf.contrib.seq2seq.LuongAttention(num_units, memory,
                              memory_sequence_length=source_sequence_length, scale=True)
