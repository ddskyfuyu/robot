...
def train(hparams, scope=None, target_session=""):
    ...
    # First evaluation
    run_full_eval(model_dir, infer_model, infer_sess,
                  eval_model, eval_sess, hparams,
    ...
    stats, info, start_train_time = before_train(loaded_train_model, train_model, 
                                                train_sess, global_step, hparams, log_f)
    ...
    (result_summary, _, final_eval_metrics) = (run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess, hparams,
    ...
    for metric in hparams.metrics:
        ...
        result_summary, best_global_step, _ = run_full_eval(best_model_dir, infer_model, infer_sess, eval_model, eval_sess, hparams,
                                                           summary_writer, sample_src_data, sample_tgt_data)
        ...
        if avg_ckpts:
            ...
            summary_writer = tf.summary.FileWriter(os.path.join(best_model_dir, summary_name), infer_model.graph)
            result_summary, best_global_step, _ = run_full_eval(best_model_dir, infer_model, infer_sess, eval_model, eval_sess,
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
    ...
    translation = nmt_utils.get_translation(nmt_outputs,
                                            sent_id=0,
                                            tgt_eos=hparams.eos,
