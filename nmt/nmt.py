...
def add_arguments(parser):
    ...
    parser.add_argument("--encoder_type", type=str, default="uni", help="uni | bi | gnmt.")
    parser.add_argument("--residual", type="bool", nargs="?", const=True, default=False, help="Whether to add residual connections.")
    parser.add_argument("--time_major", type="bool", nargs="?", const=True, default=True, help="Whether to use time-major mode for dynamic RNN.")
    parser.add_argument("--num_embeddings_partitions", type=int, default=0, help="Number of partitions for embedding vars.")

    # attention mechanisms
    parser.add_argument("--attention", type=str, default="", help="luong | scaled_luong | bahdanau | normed_bahdanau or") 
    parser.add_argument("--attention_architecture", type=str, default="standard", help="standard | gnmt | gnmt_v2")
    ...
    parser.add_argument("--warmup_scheme", type=str, default="t2t", help="How to warmup learning rates.")

    parser.add_argument("--decay_scheme", type=str, default="", help="How we decay learning rate. Options include:luong234|luong5|luong10")
    parser.add_argument("--num_train_steps", type=int, default=12000, help="Num steps to train.")
    parser.add_argument("--colocate_gradients_with_ops", type="bool", nargs="?", const=True,default=True)

    # initializer
    parser.add_argument("--init_op", type=str, default="uniform", help="uniform | glorot_normal | glorot_uniform")

    parser.add_argument("--init_weight", type=float, default=0.1,help="initialize weights between [-this, this].")
    ...
    # Vocab
    parser.add_argument("--vocab_prefix", type=str, default=None, help="Vocab prefix, expect files with src/tgt suffixes.")
    ...
    parser.add_argument("--share_vocab", type="bool", nargs="?", const=True, default=False, help="vocab and embeddings for both source and target")
    parser.add_argument("--check_special_token", type="bool", default=True, help="special sos, eos, unk tokens exist in the vocab")

    # Sequence lengths
    parser.add_argument("--src_max_len", type=int, default=50, help="Max length of src sequences during training.")
    parser.add_argument("--tgt_max_len", type=int, default=50, help="Max length of tgt sequences during training.")
    parser.add_argument("--src_max_len_infer", type=int, default=None, help="Max length of src sequences during inference.")
    parser.add_argument("--tgt_max_len_infer", type=int, default=None, help="Max length of tgt sequences during inference")
    ...
    # SPM
    parser.add_argument("--subword_option", type=str, default="", choices=["", "bpe", "spm"], help="Set to bpe or spm to activate subword desegmentation.")

    # Experimental encoding feature.
    parser.add_argument("--use_char_encode", type="bool", default=False, help="generate the word-level representation from the character")
    ...
    parser.add_argument("--steps_per_external_eval", type=int, default=None, help="How many training steps to do per external evaluation.")
    ...
    parser.add_argument("--avg_ckpts", type="bool", nargs="?",
                      const=True, default=False, help="Average the last N checkpoints for external evaluation. N can be controlled by setting --num_keep_ckpts.")
...
def extend_hparams(hparams):
    ...
    src_vocab_size, src_vocab_file = vocab_utils.check_vocab(src_vocab_file, hparams.out_dir,
                                                             ...
                                                             sos=hparams.sos, eos=hparams.eos,
                                                             unk=vocab_utils.UNK)
...
def run_main(flags, default_hparams, train_fn, inference_fn, target_session=""):
    ...
    if not loaded_hparams:  # Try to load from out_dir
        assert out_dir
        hparams = create_or_load_hparams(out_dir, default_hparams, flags.hparams_path, save_hparams=(jobid == 0))
